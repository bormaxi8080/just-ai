...
function activate(context) {
    ...
    const justBinary = config.get('justBinary', 'just');
    justAiClient = new client_1.JustAiClient(projectRoot || '', justBinary);
    // Initialize risk diagnostics provider
    riskDiagnostics = new client_1.RiskDiagnostics(context, outputChannel);
    riskDiagnostics.setClient(justAiClient);
    riskDiagnostics.setEnabled(config.get('enableRiskDiagnostics', true));
    // Initialize history tree provider
    historyProvider = new historyProvider_1.HistoryTreeProvider(context);
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(event => {
        if (event.affectsConfiguration('just-ai.enableRiskDiagnostics')) {
            const enabled = config.get('enableRiskDiagnostics', true);
        ...
        if (event.affectsConfiguration('just-ai.justBinary')) {
            const newBinary = config.get('justBinary', 'just');
            justAiClient = new client_1.JustAiClient(projectRoot || '', newBinary);
            riskDiagnostics.setClient(justAiClient);
            historyProvider.setClient(justAiClient);
        }
    }));
...
function registerCommands(context) {
    // Doctor command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.doctor', async () => {
        await runDoctor();
    }));
    // Suggest command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.suggest', async () => {
        await runSuggest();
    }));
    // Explain command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.explain', async (uri) => {
        await runExplain(uri);
    }));
    // Add command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.add', async () => {
        await runAdd();
    }));
    // Fix command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.fix', async () => {
        await runFix();
    }));
    // Workflow command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.workflow', async () => {
        await runWorkflow();
    }));
    // Fix Batch command
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.fixBatch', async () => {
        await runFixBatch();
    }));
    ...
    // Open history panel
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.openHistory', () => {
        vscode.commands.executeCommand('workbench.view.extension.just-ai');
    }));
    // Refresh history
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.refreshHistory', () => {
        historyProvider.refresh();
    }));
    // Show history detail
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.showHistoryDetail', (record) => {
        showHistoryDetail(record);
    }));
    // Configure settings
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.configure', () => {
        vscode.commands.executeCommand('workbench.action.openSettings', 'just-ai');
    }));
    // Quick pick for recipe commands
    context.subscriptions.push(vscode.commands.registerCommand('just-ai.runRecipe', async () => {
...
async function runDoctor() {
    ...
    try {
        const result = await justAiClient.runDoctor(false);
...
async function runExportContext() {
    outputChannel.clear();
    outputChannel.show();
...
async function runExplainBatch() {
    ...
    try {
        ...
        const moduleChoice = await vscode.window.showQuickPick(['All recipes', ...modules], { placeHolder: 'Select module to explain (or all recipes)' });
        if (!moduleChoice) {
            return;
        }
...
function showHistoryDetail(record) {
    const panel = vscode.window.createWebviewPanel('just-ai.historyDetail', `History: ${record.recipe}`, vscode.ViewColumn.One, { enableScripts: true });
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
