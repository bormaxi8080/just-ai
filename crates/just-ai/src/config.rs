...
impl Config {
    ...
    pub fn load(project_root: &Path) -> Result<Self, ConfigError> {
        ...
        if let Some(path) = config_path {
            let content = std::fs::read_to_string(&path)
                .map_err(|e| ConfigError { kind: ConfigErrorKind::IoError, path: path.clone(), source: e.to_string() })?;
            toml::from_str(&content).map_err(|e| ConfigError { kind: ConfigErrorKind::ParseError, path, source: e.to_string() })
        } else {
            Ok(Self::default())
        }
    }
    ...
    pub fn load_from(path: &Path) -> Result<Self, ConfigError> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| ConfigError { kind: ConfigErrorKind::IoError, path: path.to_path_buf(), source: e.to_string() })?;
        toml::from_str(&content).map_err(|e| ConfigError { kind: ConfigErrorKind::ParseError, path: path.to_path_buf(), source: e.to_string() })
    }
}
...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.kind {
            ConfigErrorKind::IoError => write!(f, "failed to read config file {}: {}", self.path.display(), self.source),
            ConfigErrorKind::ParseError => write!(f, "failed to parse config file {}: {}", self.path.display(), self.source),
        }
    }
}
...
fn default_max_record_bytes() -> usize { 64 * 1024 }
...
fn default_max_capture_bytes() -> usize { 8 * 1024 * 1024 }
fn default_stream_queue_capacity() -> usize { 32 }
fn default_cancellation_poll_ms() -> u64 { 25 }
fn default_just_binary() -> String { "just".to_string() }
...
fn default_confirm_typed_phrase() -> String { "run {recipe}".to_string() }
...
impl PolicyDecisionConfig {
    fn to_policy_decision(&self, recipe: &str, default_phrase: &str) -> crate::domain::policy::PolicyDecision {
        match self {
            ...
            PolicyDecisionConfig::ConfirmTyped { phrase } => crate::domain::policy::PolicyDecision::ConfirmTyped {
                phrase: phrase.as_ref().map(|p| p.replace("{recipe}", recipe))
...
fn default_ai_timeout_secs() -> u64 { 60 }
...
impl AiConfig {
    ...
    pub fn effective_api_key(&self) -> Option<String> {
        self.api_key.clone().or_else(|| std::env::var("JUST_AI_API_KEY").ok())
    }
...
fn default_file_budget() -> usize { 16 * 1024 }
fn default_total_budget() -> usize { 64 * 1024 }
...
fn default_max_editable_file_bytes() -> usize { 1024 * 1024 }
fn default_prefix_limit() -> usize { 16 * 1024 }
...
fn default_max_message_bytes() -> usize { 1024 * 1024 }
...
mod tests {
    ...
    fn find_config_walks_parents() {
        ...
        fs::write(dir.path().join("just-ai.toml"), "[history]\nmax_records = 42\n").unwrap();
