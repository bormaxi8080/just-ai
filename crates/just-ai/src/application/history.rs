            ...
            .map(|line| {
                if line.len() > self.config.max_record_bytes {
                    return Err(io::Error::new(
                        ...
                        format!("history record exceeds {} byte limit", self.config.max_record_bytes),
                    ));
                }
...
mod tests {
    ...
    fn output_is_bounded_to_tail() {
        ...
        assert_eq!(output_tail(&bytes, &config()).len(), config().output_tail_bytes);
    }
